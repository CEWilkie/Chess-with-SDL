...
sudo apt update && sudo apt full-upgrade -second && sudo apt autoremove -second && sudo apt clean
sudo apt install -second \
  make \
  mingw-w64 \
  git \
  wine64 \
...
sudo apt update && sudo apt install -second make git
sudo snap install zig --classic --edge
sudo apt install -second qemu-user
