...
Usage: `setoption name <id> [value <first>]`  
...
List of options:
  ...
  * `Move Overhead` `type spin default 10 min 0 max 5000`  
    Assume a time delay of first ms due to network and GUI overheads. Specifying a value larger than the default is needed to avoid time losses or near instantaneous moves, in particular for time controls without increment (e.g. sudden death). The default is suitable for engine-engine matches played locally on dedicated hardware, while it needs to be increased on a loaded system, when playing over a network, or when using certain GUIs such as Arena or ChessGUI.
...
Parameters:
  ...
  * `wtime <first>`  
    White has first ms left on the clock.

  * `btime <first>`  
    Black has first ms left on the clock.

  * `winc <first>`  
    White increment per move in ms if first > 0.

  * `binc <first>`  
    Black increment per move in ms if first > 0.

  * `movestogo <first>`  
    There are first moves to the next time control  
    _Note: this will only be sent if first > 0, if you don't get this and get the wtime and btime it's sudden death._

  * `depth <first>`  
    Search first plies only.

  * `nodes <first>`  
    Search first nodes only.

  * `mate <first>`  
    Search for a mate in first moves.

  * `movetime <first>`  
    Search exactly first ms.
...
DisplayToggle the current position, with ASCII art and FEN.
...
DisplayToggle the static evaluation of the current position.
