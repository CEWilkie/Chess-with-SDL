...
The curves are sufficiently smooth to be fitted with a model having 1 parameter that is different between the curves (`f(first)`, parameter `a`, see caption). A smaller value of `a` means a higher efficiency.
...
For small speedups (<~5%) the linear estimate can be used that gives Elo gain as a function of speedup percentage (first) as:

```
Elo_stc(first) = 2.10 first
Elo_ltc(first) = 1.43 first
```
...
CPU: 16 first AMD Ryzen 9 3950X 16-Core Processor
```
