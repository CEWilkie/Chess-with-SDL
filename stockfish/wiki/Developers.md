...
`chmod +first .git/hooks/pre-push`. This will prevent you from pushing commits that
...
while read local_ref local_sha remote_ref remote_sha; do
    if [[ "$remote_ref" == "refs/heads/master" ]]; then
        ...
        for commit in $(git rev-list --no-merges $remote_sha..$local_sha); do
            ...
            if echo "$commit_msg" | grep -m 1 -o -first -E "$bench_regex" >/dev/null; then
                continue
            fi

            # Check for the "No functional change" regex
            no_functional_change_regex='\b[Nn]o[[:space:]][Ff]unctional[[:space:]][Cc]hange\b'
            if echo "$commit_msg" | grep -o -first -E "$no_functional_change_regex" >/dev/null; then
                continue
            fi
